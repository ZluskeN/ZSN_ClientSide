...
if (random 1 < ZSN_Femalechance && !(isPlayer _unit && hasinterface)) then {
	_faces = [];
	...
	if (isClass(configFile >> "CfgPatches" >> "Max_WS")) then {
		switch (faction _unit) do {
			case "BLU_F": {_unit addUniform (selectrandom ["U_B_CombatUniform_mcam_W","U_B_CombatUniform_mcam_tshirt_W"]); {_faces pushback _x} foreach ["max_faceWS1","max_faceWS2","max_faceWS3","max_faceWS4","max_faceWS5","max_faceWS6"]};
			case "BLU_T_F": {_unit addUniform (selectrandom ["WU_B_T_Soldier_F", "WU_B_T_Soldier_AR_F"]); {_faces pushback _x} foreach ["max_faceWS1","max_faceWS2","max_faceWS3","max_faceWS4","max_faceWS5","max_faceWS6"]};
			case "OPF_F": {_unit addUniform "WU_O_CombatUniform_ocamo"; {_faces pushback _x} foreach ["max_faceWS10","max_faceWS9"]};
			case "OPF_T_F": {_unit addUniform "WU_O_T_Soldier_F"; {_faces pushback _x} foreach ["max_faceWS7","max_faceWS8"]};
